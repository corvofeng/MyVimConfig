...
# This is a copy from SpaceVim, here is the origin file: https://spacevim.org/install.sh
# and I made some modify on it.
...
# fetch_repo {{{
fetch_repo () {
  if [[ -d "$HOME/.CoolVim" ]]; then
    info "Trying to update CoolVim"
    cd "$HOME/.CoolVim"
    git pull
    cd - > /dev/null 2>&1
...
uninstall_vim () {
  if [[ -d "$HOME/.vim" ]]; then
    if [[ "$(readlink $HOME/.vim)" =~ \.CoolVim$ ]]; then
      rm "$HOME/.vim"
      success "Uninstall CoolVim for vim"
...
# check_requirements {{{
check_requirements () {
  info "Checking Requirements for CoolVim"
  ...
  if hash "vim" &>/dev/null; then
    ...
    if [ -n "$is_vim8" ]; then
      ...
      if hash "nvim" &>/dev/null; then
        ...
        warn "CoolVim need vim 7.4 or above"
      fi
    fi
    if hash "nvim" &>/dev/null; then
      success "Check Requirements: nvim"
...
usage () {
  ...
  echo " -u, --uninstall          Uninstall CoolVim"
  echo " -c, --checkRequirements  checkRequirements for CoolVim"
  echo ""
  echo "EXAMPLE"
  echo ""
...
main () {
  ...
  then
    case $1 in
      --uninstall|-u)
        info "Trying to uninstall CoolVim"
        uninstall_vim
        uninstall_neovim
